...
describe('reachability across the real store modules', () => {
	...
	it('turns unreachable without disconnecting', async () => {
		...
		await expect(new API(store.get().connection, 'win-1').getIndices()).rejects.toThrow();
	...
	it('turns reachable again on the next success', async () => {
		...
		respond({ data: [] }, true);
		await new API(store.get().connection, 'win-1').getIndices();

		expect(store.get().server.reachable).toBe(true);
	});

	it('ignores requests to another cluster, such as testing a saved connection', async () => {
		respond({ error: 'connect ECONNREFUSED', unreachable: true }, false);
		await expect(new API({ host: 'http://down.example', port: '9200' }, 'win-1').test()).rejects.toThrow();
		expect(store.get().server.reachable).toBe(true);
	});
	...
	it('keeps the flag on an error the cluster returned', async () => {
		...
		await expect(new API(store.get().connection, 'win-1').genericRequest({ path: '/missing' })).rejects.toThrow();
