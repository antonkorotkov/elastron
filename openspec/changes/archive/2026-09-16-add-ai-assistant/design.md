...
**Tool catalog.** Checked against the route handlers in `src/routes/api/elastic/**`. `auto` runs without confirmation; `confirm` gets a `'user-approval'` policy (decision 4).

| Tool | Underlying ES operation | Policy |
| --- | --- | --- |
| `list-indices` | `GET /_cat/indices[/{pattern}]?format=json`, with an optional sort such as `store.size:desc` and byte unit, so the assistant can answer "largest index" itself. Paged 50 at a time (decision 13) (same op as `/api/elastic/indices`) | auto for page 1; confirm for later pages |
...
| `run-es-request` | Arbitrary `{method, path, querystring, body, headers}`; a query string written into the path moves into `querystring`, so the URL never gets two `?` (same op as `/api/elastic/request`) | confirm, always, regardless of method |
| `propose-query` | No ES call. Takes a structured `{kind: 'search' \| 'request', index, method, path, querystring, body}` and returns it for the UI to render as a query card (decision 9). A search takes only `q`, `size`, `from`, and `sort` as parameters, which make it a URI search | auto |
...
`src/routes/api/ai/chat/+server.js` calls the `ai` package's `streamText` with the resolved provider model, the tool set, a system prompt, and an explicit `stopWhen` step limit lower than the SDK's default of 20. It returns a UI message stream consumed by the `Chat` class from `@ai-sdk/svelte`. The messages it passes to `streamText` go through the context builder in decision 12 first. The request body carries the messages, the connection and window ID for the tools, the active provider's settings (decision 3), and the cluster's version and flavor. The system prompt, built in `src/lib/server/ai/`, states the cluster version and flavor and leads with "act, don't instruct": when the user asks for a change, the model makes it with the matching tool, or `run-es-request` for operations no named tool covers. It treats the approval card as the confirmation rather than asking first in text, and gives manual steps only when asked how to do something by hand. The same point goes into every write tool's description, since models decide whether to call a tool largely from it. The prompt also tells the model to answer short questions from read tools directly. When the user asks to see search results, it runs the search, sums up what came back, and hands the same query over with `propose-query`, so the full results open in the Search view or the Playground rather than as JSON in the conversation. It also proposes queries the user wants to run or edit, validating any it hasn't run. It contains no credentials or hostnames. `src/lib/store/server.js` already holds the version; it also starts recording the flavor from the `connected` event payload.
...
Mutating tools keep their server-side `execute`, and the chat route passes `streamText` a `toolApproval` map giving each of them, and `run-es-request`, the `'user-approval'` policy. In SDK version 7 this is the approval mechanism for `streamText`; the tool-level `needsApproval` flag is deprecated and reserved for workflow agents. The stream pauses with a tool-approval request, and the drawer renders a confirmation card showing the tool, method, full path, and pretty-printed body, with destructive tools styled in red. The user's choice goes back through the `Chat` class's `addToolApprovalResponse`. On approval the server runs `execute`; on denial the model is told the action was declined.
Because the server rebuilds the conversation from messages the renderer sends, approvals are signed with the SDK's `experimental_toolApprovalSecret`, using a random secret generated once per server process. A forged or altered approval is rejected before any write runs. An approval requested before an app restart can't be approved after it; the card reports the failure and the user asks again.
...
Connections have no stable ID, so each conversation is stored under a key derived from the connection's `host`, `port`, and `user`, plus the SSH tunnel's host, port, and user when one is used; the shared `endpointOf` helper in `src/lib/utils/endpoint.js` computes it. Profiles pointing at the same cluster share history, and editing a profile's name, color, headers, or password keeps it. The assistant store loads the conversation for the new endpoint on every change to the window's connection, not only on connect events, because Quick Connect updates the connection before opening a tunnel and a failed tunnel ends with neither event. Each chat remembers its endpoint and refuses to send, including the automatic send after an approval, when the connection now reaches another cluster; the composer is held until the new conversation loads. Every append trims from the oldest end to the most recent N messages before persisting. Tool results are persisted in their truncated form, so a single message can't carry a large payload into storage.

**8. The Settings modal reuses the existing modal context and tab layout.**
`SettingsDialog` opens through `getContext('modal-window').open()`, like `ConnectionDialog`. Its section list reuses the index view's vertical tab sidebar. Sections are a list of `{ id, title, component }`, so adding one doesn't touch the others. Like the app's other dialogs, the footer has a black Cancel and a green Save. Each section edits a local draft and exposes a `save()` function; Save calls it on every section and closes, while Cancel, Escape, or a click outside discard the drafts. The AI section's `save()` commits its whole draft through a single `aiSettings/save` action that writes to disk at once, replacing per-keystroke writes. The section builds its draft only once `aiSettingsLoaded` is true, showing a loading state before that, so a Save made before startup loading finishes can't overwrite the stored keys with defaults.

**9b. Approval cards expire, destructiveness is judged from the request, and retries continue.**
An approval card in any message but the latest shows as expired, since the SDK only continues from the latest message and the server drops unanswered approvals from earlier turns. The red styling and the irreversibility warning come from the built request (any DELETE, or delete- or update-by-query), so the generic request tool gets them too, and a sweeping target such as `logs-*` is named on the card. `wipe-index` checks `GET /_alias/{name}` first and refuses an alias, because a delete-by-query on an alias empties every index behind it. Retry continues a failed reply that already ran a tool or had an approval answered, by resending the conversation as it is; only a reply that failed before doing anything is regenerated, since regenerating deletes the reply along with writes that already ran.

**9c. Replies never load remote content.**
Markdown images are disabled in markdown-it and `img` is forbidden in DOMPurify, so a reply can't make the page fetch a URL. Without this, a prompt injection in cluster data could have the model write cluster data into an image URL and leak it with no click. Links still render and open only on a click.
...
- Open in Search, only for `kind: 'search'`, dispatches `search/tabs/add` and switches to the Search view without running the query. A body search opens as `{ type: 'body', index, requestBody }`. A URI search, one with `q`, `size`, `from`, or `sort` parameters, opens in the Search view's URI mode as `{ type: 'uri', index, uriQuery, size, from, sort }`. Any other parameter, or a body alongside `q`, is refused with a message telling the model what to change. The existing `MAX_TABS` refusal and its notification apply.
- Load into Playground dispatches `playground/loadTemplate` with the method, path, body, and headers, then switches to the Playground view. The Playground has one URL field, so query parameters are appended to the path there. Like loading a template, it replaces the current draft.
- Copy puts the request on the clipboard, parameters included.
The card, Copy, and Playground all format parameters with the catalog's shared helper, which keeps them readable (`?v&s=store.size:desc`) rather than percent-encoded.

**9a. Rejected tool input is muted.**
When a tool's input schema refuses what the model sent, the SDK returns the error to the model, which usually retries correctly. The UI recognizes these parts by their unparsed `rawInput` or their "Invalid input for tool" error text and shows a muted, collapsed line instead of a red failure or a failed approval card. Errors from Elasticsearch, thrown by a tool's execute, stay red.

**9d. The drawer floats over the page.**
The drawer is fixed to the right edge below the header and overlays the page rather than docking beside it. Docking narrowed the workspace by the drawer's width, which left some views without room to render. Dialogs and notifications stack above the drawer, so a Settings dialog opened from it covers it.
...
**11. Analytics sees only that a message was sent and answered.**
The drawer dispatches `assistant/messageSent` when the user sends a message and `assistant/responseReceived` when the first reply after that send finishes normally; the analytics store module turns them into the parameter-free `assistant_message_sent` and `assistant_response_received` events, the same listen-and-report pattern as `cluster_connected`. A continuation after an approval doesn't count again, and a failed or stopped reply isn't counted until a retry succeeds. The drawer is not a route, so it produces no page views, and no chat, tool, provider, or model data reaches `trackPageView` or `gtag`.
...
- Drop tool calls, tool results, approvals, and reasoning from every turn before the current one, keeping the assistant's text replies, which already summarize what the tools found. Tool traffic goes through the SDK's `pruneMessages`; reasoning is removed by hand, because `pruneMessages` can only keep reasoning in the very last message, and the current turn needs all of it. A continuation after an approval ends with the approval, not the reply, and OpenAI rejects a tool call or reply sent without the reasoning item it came with.
- Strip provider metadata from the parts left in earlier turns. For OpenAI it holds the ids of stored items, and the SDK sends a part that has one as a reference to the stored item. A reference to a reply whose reasoning was removed fails with "Item 'msg_…' of type 'message' was provided without its required 'reasoning' item", and references stop working once OpenAI drops stored items. Without the ids, earlier replies are sent as plain text. The renderer strips the same metadata from earlier turns before sending.
Tool-approval requests and responses sit in the most recent messages, so pruning never removes an approval in flight. The renderer applies the same rule before sending: tool calls, results, and reasoning from turns before the current one are stripped from the request, keeping the assistant's text. Sending them made long conversations exceed the packaged server's default 512K request limit, which surfaced as "Invalid JSON payload". As a backstop, `main.js` sets `BODY_SIZE_LIMIT` to 50M for the forked server, which only serves the app's own windows on localhost. The stored conversation and the drawer's display are untouched. Where the provider supports it, prompt caching is enabled through provider options, marking the system prompt and tool definitions as cacheable for Anthropic; OpenAI and Gemini cache repeated prefixes on their own.
...
**13. The index listing is paged, and later pages need approval.**
The index listing is the one list where a 50-row ceiling blocks basic questions, since a cluster can have thousands of indices. Elasticsearch's `_cat/indices` has no paging of its own, so `list-indices` takes a `page` argument: the server fetches the full list, sorted, on each call and returns one 50-entry page with the total and page count. Re-fetching keeps the order stable across pages and costs little, since only the page reaches the model. Page 1 runs without asking. Any later page gets the `'user-approval'` policy through a per-input rule in the `toolApproval` map, because each page sends another batch of cluster data to the provider. The rule, the page size, and the card's line naming which entries a page sends all come from the shared catalog, so the server and the card agree. The card shows for any tool call that went through approval, not only write tools.
Alternative considered: raise the ceiling instead. Rejected, because it grows every result sent to the provider and still fails on large clusters.
...
- Conversations for SSH-tunneled connections move to a new key that includes the tunnel, so history saved for them before this change isn't found. Keys for connections without a tunnel are unchanged.
