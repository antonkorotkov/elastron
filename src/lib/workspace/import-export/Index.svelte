<script>
	...
	const onSourceFileSelected = file => {
		// file is a File object
		if (file) {
			dispatch('ie/input/file', file.name)
			dispatch('ie/input/fileObject', file)
		}
	}

	const onDestinationDirSelected = handle => {
		// handle is FileSystemFileHandle
		if (handle) {
			dispatch('ie/output/file', handle.name)
			dispatch('ie/output/fileHandle', handle)
		}
	}
